...
def show_quick_status():
    ...
    for service_name, service_info in SERVICES.items():
        ...
        # Restart count
        total_restarts = sum(
            c.get("restart_count", 0) for c in status.get("containers", [])
        )
    ...
    if worker_health is not None:
        ...
        total_failed = sum(
            q.get("failed_count", 0) for q in worker_health["queues"].values()
        )
        if wc == 0:
            console.print(
    ...
    console.print("\n[dim]Legend:[/dim]")
    console.print(
        "[dim]  Containers: 🟢 Running | 🟡 Partial | 🔴 Stopped | ⚪ Not Configured | ⚫ Error[/dim]"
    )
    console.print(
        "[dim]  Restarts: 0 = stable | ⚠️  N = container crashed N times (restart loop)[/dim]"
    )
...
def show_detailed_status():
    ...
    for service_name, service_info in SERVICES.items():
        ...
        for container in status.get("containers", []):
            ...
            health_status = (
                f" ({container['health']})" if container["health"] != "none" else ""
            )
            restart_count = container.get("restart_count", 0)
            restart_info = (
                f" [bold red]⚠️  {restart_count} restarts[/bold red]"
                if restart_count > 0
                else ""
            )
            console.print(
                f"      {state_icon} {container['name']}: {container['status']}{health_status}{restart_info}"
            )
            ...
            if status["health"].get("healthy"):
                ...
                if service_name == "backend" and status["health"].get("data"):
                    ...
                    if overall_status == "healthy":
                        ...
                        console.print(
                            f"      Overall: [yellow]{overall_status}[/yellow]"
                        )
                    else:
                    ...
                    for svc_name in ["mongodb", "redis"]:
                        if svc_name in services:
                            ...
                            if svc.get("healthy"):
                                console.print(
                                    f"        [green]✅ {svc_name}: {svc.get('status', 'ok')}[/green]"
                                )
                            else:
                                console.print(
                                    f"        [red]❌ {svc_name}: {svc.get('status', 'error')}[/red]"
                                )
                    ...
                    for svc_name in optional_services:
                        if svc_name in services:
                            ...
                            if svc.get("healthy"):
                                ...
                                console.print(
                                    f"        [yellow]⚠️  {svc_name}: {svc.get('status', 'degraded')}[/yellow]"
                                )
                    ...
                    if config:
                        ...
                        console.print(
                            f"        LLM: {config.get('llm_provider', 'unknown')} ({config.get('llm_model', 'unknown')})"
                        )
                        console.print(
                            f"        Transcription: {config.get('transcription_service', 'unknown')}"
                        )
                        console.print(
                            f"        Active Clients: {config.get('active_clients', 0)}"
                        )
            else:
...
Examples:
  ...
  ./status.sh --json       Output status in JSON format
    ...
    parser.add_argument(
        "--json", "-j", action="store_true", help="Output status in JSON format"
    )
