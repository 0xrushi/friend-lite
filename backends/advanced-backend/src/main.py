...
def _new_local_file_sink(file_path: str) -> LocalFileSink:
    ...
    Returns:
        LocalFileSink: Configured file sink for writing audio.
    """
...
async def _open_file_sink_properly(sink: LocalFileSink) -> LocalFileSink:
    """
    Open a file sink and ensure all wave parameters are set correctly.

    Args:
        sink (LocalFileSink): The file sink to open.

    Returns:
        LocalFileSink: The opened and configured file sink.
    """
    if not isinstance(sink, LocalFileSink):
        raise TypeError("sink must be a LocalFileSink instance")
    await sink.open()
...
async def _process_audio_cropping_with_relative_timestamps(
    ...
    Args:
        ...
        speech_segments (List[Tuple[float, float]]): List of (start, end) tuples in absolute time.
        output_path (str): Path to save the cropped audio file.
        audio_uuid (str): Unique identifier for the audio chunk.
    ...
    if not isinstance(speech_segments, list):
        raise TypeError("speech_segments must be a list of tuples")
...
async def _crop_audio_with_ffmpeg(
    ...
    Args:
        original_path (str): Path to the original audio file.
        speech_segments (List[Tuple[float, float]]): List of (start, end) tuples in seconds.
        output_path (str): Path to save the cropped audio file.
...
async def speaker_worker() -> None:
    ...
    Continuously processes tasks from the speaker queue if the speaker service is available.
    """
    if not SPEAKER_SERVICE_AVAILABLE:
        audio_logger.info(
            "Speaker service not available - speaker worker will not process tasks"
        )
        return
    while True:
        try:
            ...
            if not isinstance(wav_path, str):
                raise TypeError("wav_path must be a string")
            if not isinstance(audio_uuid, str):
                raise TypeError("audio_uuid must be a string")
...
class TranscriptionManager:
    ...
    def __init__(self, action_item_callback=None):
        ...
        self.action_item_callback = (
            action_item_callback  # Callback to queue action items
        )
...
class ClientState:
    ...
    async def _close_current_conversation(self):
        ...
        if self.file_sink:
            ...
            current_path = (
                self.file_sink._file_path.name
                if hasattr(self.file_sink, "_file_path")
                else None
            )
            ...
            if self.conversation_transcripts and current_uuid:
                ...
                try:
                    ...
                    if action_item_count > 0:
                        audio_logger.info(
                            f"🎯 Extracted {action_item_count} action items from conversation {current_uuid}"
                        )
                    else:
                        audio_logger.info(
                            f"ℹ️ No action items found in conversation {current_uuid}"
                        )
            ...
            if current_uuid and current_path:
                ...
                if SPEAKER_SERVICE_AVAILABLE:
                    ...
                    audio_logger.info(
                        f"🎭 Queued {current_path} for speaker processing"
                    )
                else:
                    audio_logger.debug(
                        f"Speaker service not available - skipping speaker processing for {current_path}"
                    )
        else:
            audio_logger.info(
        ...
        try:
            while self.connected:
                ...
                if self.transcription_manager is None:
                    ...
                    async def action_item_callback(
                        transcript_text, client_id, audio_uuid
                    ):
    ...
    async def _action_item_processor(self):
        ...
        try:
            while self.connected:
                transcript_text, client_id, audio_uuid = (
                    await self.action_item_queue.get()
                )
                ...
                try:
                    ...
                    if action_item_count > 0:
                        audio_logger.info(
                            f"🎯 Extracted {action_item_count} action items from transcript segment for {audio_uuid}"
                        )
                    else:
                        audio_logger.debug(
                            f"ℹ️ No action items found in transcript segment for {audio_uuid}"
                        )

                except Exception as e:
                    audio_logger.error(
                        f"❌ Error processing action items for transcript segment in {audio_uuid}: {e}"
                    )
...
async def create_client_state(client_id: str) -> ClientState:
    ...
    Returns:
        ClientState: The created and registered client state.
    """
...
async def cleanup_client_state(client_id: str) -> None:
    """
    Clean up and remove client state.

    Args:
        client_id (str): The unique identifier for the client to clean up.
    """
    if not isinstance(client_id, str):
        raise TypeError("client_id must be a string")
    if client_id in active_clients:
...
async def ws_endpoint(ws: WebSocket, user_id: Optional[str] = Query(None)):
    ...
    audio_logger.info(
        f"🔌 WebSocket connection accepted - Client: {client_id}, User ID: {user_id}"
    )
...
async def ws_endpoint_pcm(ws: WebSocket, user_id: Optional[str] = Query(None)):
    ...
    try:
        ...
        while True:
            ...
            if packet:
                audio_logger.debug(
                    f"🎵 Received PCM packet #{packet_count}: {len(packet)} bytes"
                )
                chunk = AudioChunk(
                    audio=packet,
                    rate=16000,
...
async def enroll_speaker(request: SpeakerEnrollmentRequest):
    ...
    if not SPEAKER_SERVICE_AVAILABLE:
        return JSONResponse(
            status_code=503,
            content={
                "error": "Speaker service is not available. Please set SPEAKER_SERVICE_URL environment variable."
            },
        )
    ...
    try:
        ...
        if success:
            ...
            return JSONResponse(
                status_code=500, content={"error": "Failed to enroll speaker"}
            )
...
async def remove_speaker(speaker_id: str):
    ...
    try:
        ...
        if success:
            ...
            return JSONResponse(
                content={"message": f"Speaker {speaker_id} removed successfully"}
            )
        else:
            return JSONResponse(
                status_code=404, content={"error": f"Speaker {speaker_id} not found"}
            )
...
async def identify_speaker_from_file(request: SpeakerIdentificationRequest):
    ...
    try:
        ...
        if (
            not speaker_recognition.audio_loader
            or not speaker_recognition.embedding_model
        ):
            return JSONResponse(
                status_code=503,
        ...
        if request.start_time is not None and request.end_time is not None:
            ...
            waveform, _ = speaker_recognition.audio_loader.crop(
                str(full_audio_path), segment
            )
        else:
        ...
        if identified_speaker:
            # Get speaker info
            speaker_info = await speakers_col.find_one(
                {"speaker_id": identified_speaker}, {"_id": 0}
            )
            ...
            return JSONResponse(
                content={"identified": False, "message": "No matching speaker found"}
            )
...
async def get_action_items(user_id: str, status: Optional[str] = None, limit: int = 50):
    ...
    try:
        action_items = await action_items_service.get_action_items(
            user_id=user_id, limit=limit, status_filter=status
        )
        return JSONResponse(
            content={
        ...
        return JSONResponse(
            status_code=500, content={"error": "Failed to fetch action items"}
        )
...
async def search_action_items(user_id: str, query: str, limit: int = 20):
    ...
    try:
        action_items = await action_items_service.search_action_items(
            query=query, user_id=user_id, limit=limit
        )
        return JSONResponse(
            content={
        ...
        return JSONResponse(
            status_code=500, content={"error": "Failed to search action items"}
        )
...
async def create_action_item(user_id: str, request: ActionItemCreateRequest):
    ...
    try:
        ...
        if action_item:
            ...
            return JSONResponse(
                status_code=500, content={"error": "Failed to create action item"}
            )
...
async def update_action_item_status(
    action_item_id: str, request: ActionItemUpdateRequest
):
    ...
    try:
        ...
        if success:
            ...
            return JSONResponse(
                status_code=404, content={"error": "Action item not found"}
            )

    except Exception as e:
        audio_logger.error(f"Error updating action item {action_item_id}: {e}")
        return JSONResponse(
            status_code=500, content={"error": "Failed to update action item"}
        )
...
async def delete_action_item(action_item_id: str):
    ...
    try:
        ...
        return JSONResponse(
            status_code=500, content={"error": "Failed to delete action item"}
        )
...
async def get_action_item_stats(user_id: str):
    ...
    try:
        ...
        return JSONResponse(
            status_code=500, content={"error": "Failed to get action item statistics"}
        )
...
async def extract_action_items_from_conversation(audio_uuid: str):
    ...
    try:
        ...
        audio_logger.error(
            f"Error extracting action items from conversation {audio_uuid}: {e}"
        )
        return JSONResponse(
            status_code=500, content={"error": "Failed to extract action items"}
        )
