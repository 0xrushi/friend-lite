...
def normalize_embedding(embedding: np.ndarray) -> np.ndarray:
    ...
    Args:
        embedding (np.ndarray): The embedding to normalize.
...
def enroll_speaker(
    ...
    Args:
        ...
        speaker_name (str): Human-readable name for the speaker.
        audio_file (str): Path to audio file.
        start_time (Optional[float]): Start time in seconds (optional).
        end_time (Optional[float]): End time in seconds (optional).
    ...
    if not audio_loader or not embedding_model:
        log.error(
            "Audio loader or embedding model not available for speaker enrollment"
        )
        return False
    try:
        # Load audio
        ...
        for existing_speaker in enrolled_speakers:
            if existing_speaker["id"] == speaker_id:
                log.warning(
                    f"Speaker {speaker_id} already enrolled. Updating embedding."
                )
                ...
                if enrolled_speakers:
                    embeddings = np.vstack(
                        [spk["embedding"] for spk in enrolled_speakers]
                    )
...
def identify_speaker(embedding: np.ndarray) -> Optional[str]:
    ...
    Args:
        embedding (np.ndarray): Speaker embedding to identify.

    Returns:
        Optional[str]: Speaker ID if identified, None if no match.
    """
...
def list_enrolled_speakers() -> List[Dict]:
    ...
    Returns:
        List[Dict]: List of dictionaries with speaker id and name.
    """
...
def remove_speaker(speaker_id: str) -> bool:
    ...
    Args:
        speaker_id (str): The ID of the speaker to remove.

    Returns:
        bool: True if the speaker was removed, False otherwise.
    """
...
async def process_file(wav_path: Path, audio_uuid: str, mongo_chunks) -> None:
    """
    Process audio file for speaker diarization and identification.

    Args:
        wav_path (Path): Path to WAV file.
        audio_uuid (str): Unique identifier for this audio.
    ...
    if not isinstance(wav_path, Path):
        raise TypeError("wav_path must be a pathlib.Path instance")
    ...
    if not diar or not audio_loader or not embedding_model:
        log.warning(
            "Speaker recognition models not available, skipping processing for %s",
            audio_uuid,
        )
        return
    try:
        ...
        for spk in diar_result.labels():
            try:
                ...
                if identified_speaker:
                    ...
                    log.info(
                        f"Unknown speaker {spk}, assigned ID: {identified_speaker}"
                    )
        ...
        await mongo_chunks.update_one(
            {"audio_uuid": audio_uuid},
            {
                ...
                "$addToSet": {
                    "speakers_identified": {"$each": list(speakers_identified)}
                },
            },
        )
        log.info(
            "Speaker diarization completed for %s with %d speakers",
            audio_uuid,
            len(speakers_identified),
        )
    except Exception as e:
