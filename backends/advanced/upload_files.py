...
def get_admin_token(
    password: str, base_url: str = "http://localhost:8000"
) -> Optional[str]:
...
def collect_wav_files(
    audio_dir: str, filter_list: Optional[list[str]] = None
) -> list[str]:
    ...
    for file_path in candidate_files:
        ...
        if not is_valid:
            logger.error(
                f"🔴 SKIPPING: {file_path.name} - Invalid format: {format_error}"
            )
            continue
        ...
        logger.info(
            f"✅ Added file: {file_path.name} (duration: {duration_minutes:.1f} minutes)"
        )

    total_minutes = total_duration / 60.0
    logger.info(
        f"📊 Total files to upload: {len(selected_files)} (total duration: {total_minutes:.1f} minutes)"
    )
...
def upload_files_async(
    files: list[str], token: str, base_url: str = "http://localhost:8000"
) -> bool:
    ...
    while True:
        try:
            ...
            if progress != last_progress or current_file != last_current_file:
                ...
                if current_file:
                    logger.info(
                        f"📈 Progress: {progress:.1f}% ({processed_files}/{total_files}) - Processing: {current_file} (elapsed: {elapsed:.0f}s)"
                    )
                else:
                    logger.info(
                        f"📈 Progress: {progress:.1f}% ({processed_files}/{total_files}) (elapsed: {elapsed:.0f}s)"
                    )
            ...
            if status == "finished":
                ...
                for file_info in files:
                    if file_info.get("status") == "failed":
                        ...
                        logger.warning(
                            f"   ⏭️  {file_info.get('filename')}: {error_msg}"
                        )
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Upload audio files to Chronicle backend"
    )
    parser.add_argument(
        "files",
        nargs="*",
...
def main():
    ...
    if args.files:
        ...
        specific_file = (
            ...
            / "extras"
            / "test-audios"
            / "DIY Experts Glass Blowing_16khz_mono_4min.wav"
        )
        ...
        if specific_file.exists():
            ...
            logger.info(
                "Please provide file paths as arguments or ensure test file exists"
            )
            sys.exit(1)
