    ...
    def setup_https(self):
        ...
        if enable_https:
            ...
            if ssl_script.exists():
                try:
                    subprocess.run(
                        [str(ssl_script), server_ip], check=True, timeout=180
                    )
                    self.console.print(
                    ...
                    self.console.print(
                        "[yellow][WARNING][/yellow] SSL certificate generation timed out"
                    )
                ...
                self.console.print(
                    f"[yellow][WARNING][/yellow] SSL script not found at {ssl_script}"
                )
            ...
            if nginx_template.exists():
                try:
                    ...
                    self.console.print(
                        f"[green][SUCCESS][/green] nginx.conf created for: {server_ip}"
                    )
                except Exception as e:
                    self.console.print(
                        f"[yellow][WARNING][/yellow] nginx.conf generation failed: {e}"
                    )
            else:
                self.console.print(
                    "[yellow][WARNING][/yellow] nginx.conf.template not found"
                )
            ...
            self.console.print(
                "   ⚠️  Note: Microphone access may not work over HTTP on remote connections"
            )
